...
# Arrays of words for each difficulty level, you can add words here if you pertain to the format.
# Difficulty 1
difficulty_1_words=(
...
# Generate hack lines for the game, change this number (19 default to make more "hacklines")
hack_lines=()
for ((i = 0; i < 19; i++)); do
...
# Randomly choose a set number of hack lines from the generated hack lines [WORDS] Change this number to make more words generate per game (same as rest with [WORDS])
selected_hacklines=()
used_hacklines=()
for ((i = 0; i < 10; i++)); do
...
# Generate the selected # words [WORDS] Change this number to make more words generate per game (same as rest with [WORDS])
selected_words=()
...
# Concatenate the selected hack lines and the selected words, [WORDS] Change this number to make more words generate per game (same as rest with [WORDS])
for ((i = 0; i < 10; i++)); do
  hack_line="${selected_hacklines[i]}"
  selected_word="${selected_words[i]}"
  position_choice=$((RANDOM % 2))

# Where words are placed within "hacklines"
...
# Remove the pound sign to enable developer mode, do same for rest of instances
...
while [[ $attempts -gt 0 ]]; do
    ...
    echo " "
    read -p "C> " input
    play -q $FO3_TERM_BASE_DIR/enter.wav
